...
class CurrencyListUtilsTest {
    ...
    fun testCalculateBaseValue() {
        assertEquals(
            "Should equal 1.1",
            CurrencyListUtils.calculateBaseValue(1.1, Currency("USD", 1.0)),
            1.1,
            delta
        )
        assertEquals(
            "Should equal 3.0",
            CurrencyListUtils.calculateBaseValue(15.0, Currency("USD", 5.0)),
            3.0,
            delta
        )
        assertEquals(
            "Should equal 1.1",
            CurrencyListUtils.calculateBaseValue(1.1, null),
            1.1,
            delta
        )
    }
    ...
    fun testParseResponseFromJson() {
        ...
        assertEquals("Base currency should equal EUR", response!!.base, "EUR")
        assertEquals("Currencies count should equal 32", response.rates.size, 32)
    }
    ...
    fun testGenerateListFromResponse() {
        ...
        assertEquals("First currency name should equal EUR", list.first().name, "EUR")
        assertEquals("Currency list size should equal 33", list.size, 33)
    }
    ...
    fun testGetValidBaseCurrency() {
        ...
        val list = CurrencyListUtils.generateListFromResponse(response!!)
        assertEquals(
            "Currency should equal EUR",
            CurrencyListUtils.getValidBaseCurrency(1.0, null, list).name,
            "EUR"
        )
        assertEquals(
    ...
    fun testCalculateCorrespondingValues() {
        val response = CurrencyListUtils.parseResponseFromJson(sampleResponce1)
        val oldList = CurrencyListUtils.generateListFromResponse(response!!)
        val newList = CurrencyListUtils.calculateCorrespondingValues(2.0, oldList)

        oldList.first().calculateValue(2.0)
        assertEquals(oldList.first().value, newList.first().value, delta)

        oldList[5].calculateValue(2.0)
        assertEquals(oldList[5].value, newList[5].value, delta)
    }
